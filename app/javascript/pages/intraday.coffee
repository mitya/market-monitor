...
makeChart = (ticker, data) ->
  chartsContainer().insertAdjacentHTML('beforeend', "<div class='intraday-chart'></div>")
  ...
  lineSeries.setData candles.map (row) -> { time: row[0], open: row[1], high: row[2], low: row[3], close: row[4] }
  ...
  volumeSeries.setData candles.map (row) -> { time: row[0], value: row[5] }  
...
document.addEventListener "turbolinks:load", ->
  if document.querySelector('.intraday-charts')
    ...
    # chart = createChart container, width: 0, height: 300, timeScale: { timeVisible: true, secondsVisible: false }
    # lineSeries = chart.addCandlestickSeries()
    # lineSeries.setData data['VEON'].map (row) -> { time: row[0], open: row[1], high: row[2], low: row[3], close: row[4] }
    # 
    # volumeSeries = chart.addHistogramSeries({
    #   priceFormat: { type: 'volume' }, priceLineVisible: false, color: 'rgba(76, 76, 76, 0.5)', priceScaleId: '', scaleMargins: { top: 0.85, bottom: 0 },
    # })
    # volumeSeries.setData data['VEON'].map (row) -> { time: row[0], value: row[5] }
    # 
    
    # chart = createChart document.getElementById('chart-2'), width: 0, height: 300
    ...
    # 
    # 
    # chart = createChart document.getElementById('chart-3'), width: 0, height: 300
    ...
    #   { time: '2019-05-22', value: 19103293.00 },
    #   { time: '2019-05-23', value: 21737523.00 },
    #   { time: '2019-05-24', value: 29328713.00 },
    #   { time: '2019-05-28', value: 37435638.00 }
    # ])
